...
export default function Admin() {
  ...
  async function approve(id) {
    try {
      ...
      setSubmissions((s) =>
        s.map((x) => (x._id === id ? { ...x, status: "approved" } : x)),
      );
    } catch (e) {
  ...
  async function reject(id) {
    try {
      ...
      setSubmissions((s) =>
        s.map((x) => (x._id === id ? { ...x, status: "rejected" } : x)),
      );
    } catch (e) {
      flash(e.response?.data?.message || "Error");
    }
  ...
  async function addCandidate(e) {
    ...
    const categoryToUse =
      candForm.category === "__new__"
        ? candForm.newCategory
        : candForm.category;
    if (!candForm.name || !categoryToUse)
      return flash("Name and category are required.");

    // If adding a new category, create it first
    if (candForm.category === "__new__") {
      try {
        await adminAxios(token).post("/api/categories", {
          name: categoryToUse,
        });
      } catch (e) {
  ...
  async function addCategory(e) {
    ...
    try {
      const res = await adminAxios(token).post("/api/categories", {
        name: catName,
      });
      ...
      flash(e.response?.data?.message || "Error adding category");
    }
  }
  ...
  if (!isLoggedIn) {
    return (
      ...
      >
        ...
        >
          <h2
            style={{ textAlign: "center", marginBottom: 8, color: "#C9A84C" }}
          >
            Admin Panel
          </h2>
          <p
          ...
          <form onSubmit={handleLogin}>
            ...
            {loginError && (
              <div style={{ color: "#E05A5A", fontSize: 14, marginBottom: 16 }}>
                {loginError}
              </div>
            )}
            <button
    ...
    <div className="container" style={{ padding: "32px 24px" }}>
      ...
      >
        <div>
          ...
          <p style={{ color: "#9A9488", fontSize: 14 }}>
            Manage votes, candidates & categories
          </p>
        </div>
      ...
      {msg && (
        <div
          style={{
            background: "rgba(76,175,126,0.1)",
            border: "1px solid rgba(76,175,126,0.3)",
            color: "#4CAF7E",
            borderRadius: 8,
            padding: "12px 16px",
            marginBottom: 24,
      ...
      {tab === "submissions" && (
        <div>
          {loading ? (
            <div style={{ color: "#9A9488", padding: 40 }}>Loading...</div>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      <div
                        style={{ fontSize: 13, color: "#9A9488", marginTop: 2 }}
                      >
                        Ref:{" "}
                        <span
                          style={{ color: "#F0EDE4", fontFamily: "monospace" }}
                        >
                          {s.transactionRef}
                        </span>
                      </div>
                      <div
                        style={{ fontSize: 12, color: "#9A9488", marginTop: 4 }}
                      >
      ...
      {tab === "candidates" && (
        ...
        >
            ...
            <form onSubmit={addCandidate}>
              <div className="form-group">
                ...
                <input
                  ...
                  value={candForm.name}
                  onChange={(e) =>
                    setCandForm((f) => ({ ...f, name: e.target.value }))
                  }
                />
              </div>
              <div className="form-group">
                ...
                <select
                  value={candForm.category}
                  onChange={(e) =>
                    setCandForm((f) => ({ ...f, category: e.target.value }))
                  }
                >
          ...
          <div>
            ...
            {candidates.length === 0 ? (
              <div
                style={{ color: "#9A9488", padding: 40, textAlign: "center" }}
              >
                No candidates yet.
              </div>
            ) : (
